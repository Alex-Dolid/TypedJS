...
describe('BaseTJS', () => {
  ...
  describe('BaseTJS when strict mode is off', () => {
    ...
    describe('[Method] getType when strict mode is off', () => {
      getTestCases(fixtures.nonStrict).forEach(([name, args, expected]) => {
        it(name, () => {
          expect(baseTJS.getType(...args)).toBe(expected);
        });
      });
    });

    describe('[Method] getType when strict mode is on', () => {
      getTestCases(fixtures.strict, { strict: true }).forEach(
        ([name, args, expected]) => {
          it(name, () => {
            expect(baseTJS.getType(...args)).toBe(expected);
          });
  ...
  describe('BaseTJS when strict mode is on', () => {
    ...
    describe('[Method] getType when strict mode is off', () => {
      getTestCases(fixtures.nonStrict, { strict: false }).forEach(
        ([name, args, expected]) => {
          it(name, () => {
            expect(baseTJS.getType(...args)).toBe(expected);
          });
    ...
    describe('[Method] getType without options (should works like when strict mode is on)', () => {
      getTestCases(fixtures.strict).forEach(([name, args, expected]) => {
        it(name, () => {
          expect(baseTJS.getType(...args)).toBe(expected);
        });
      });
