...
const getStartMessage = (value) =>
  typeof value === TYPES.STRING ? `"${value}"` : `${value}`;
        ...
        it(getItTestCaseName(i, Type, testCase, 'correct message'), () => {
          expect(BaseTJS.getTypeErrorMessage(testCase, 'mytype')).toBe(
            BaseTJS.isPrimitive(testCase) && typeof testCase !== TYPES.SYMBOL
              ? `TypedJS: ${getStartMessage(testCase)} is not a mytype`
              : `TypedJS: ${BaseTJS.capitalize(
                  BaseTJS.typeOf(testCase),
                )} is not a mytype`,
          );
        });
      });
